...
#define MPU_IIC_Init MX_I2C2_Init
...
u8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		//accel_sens=0;//��������  ��ֹ��λ��С����ƽ�⣬��MPU6050����ֵΪ0  --zzw 2024.6.10
...
// static KalmanFilter Kalman_Struct[3] = {{0.02,0,0,0,0.001,0.8},{0.02,0,0,0,0.001,0.8},{0.02,0,0,0,0.001,0.8}};
static volatile int16_t accel_databuff[3]={0};//角加速度数据暂存
static volatile int16_t gyro_databuff[3]={0};//角速度数据暂存


void MPU6050_Init(void)
{
    int16_t t=0;
    int32_t buffer_gyro[3]={0};
    int32_t buffer_accel[3]={0};
    while(mpu_dmp_init()){
        HAL_Delay(200);
...
/*mpu6050校准值*/
static float calibration_value_angle[3]={0,0,0};//角度校准值
static int16_t calibration_value_gyro[3]={0,0,0};//角速度校准值
static int16_t calibration_value_accel[3]={0,0,0};//角加速度校准值

u8 MPU_Get_Gyroscope(short *gx,short *gy,short *gz)
{
    
    volatile static short Gyro_t[3][10]={0};
    u8 buf[6],res; 
    short gx_t,gy_t,gz_t; 
	res=MPU_Read_Len(MPU_Addr,0X43,6,buf);
    #if 1
        if(res==0)
        {
            gx_t=((u16)buf[0]<<8)|buf[1];  
            gy_t=((u16)buf[2]<<8)|buf[3];  
            gz_t=((u16)buf[4]<<8)|buf[5];
        } 	
    #endif
    /*对角速度进行低通滤波*/
    #if 1
        static volatile short tbuf[3]={0};
        static volatile float filter_Param=0.25f;
        gx_t=tbuf[0]=(short)(gx_t*filter_Param+tbuf[0]*(1-filter_Param));
        gy_t=tbuf[1]=(short)(gy_t*filter_Param+tbuf[1]*(1-filter_Param));
        gz_t=tbuf[2]=(short)(gz_t*filter_Param+tbuf[2]*(1-filter_Param));
        *gx=gx_t-calibration_value_gyro[0];
        *gy=gy_t-calibration_value_gyro[1];
        *gz=gz_t-calibration_value_gyro[2];
    #endif
...
u8 MPU_Get_Accelerometer(float *ax,float *ay,float *az)
{
	...
	res=MPU_Read_Len(MPU_Addr,0X3B,6,buf);
    #if 1
        if(res==0)
        {
    ...
    /*对角加速度进行卡尔曼滤波*/
        ...
        kalman_Filter(&Kalman_Struct[1],(float)accel_databuff[1]);
        accel_databuff[1]=(int16_t)Kalman_Struct[1].out;
...
void MPU6050_Calibration(int16_t aacx,int16_t aacy,int16_t aacz,short gyrox,short gyroy,short gyroz)//mpu6050校正函数
{
  static uint16_t Calibration_times=0; 
  if(Calibration_times > 300)return;
  /*mpu6050数据校准*/
  volatile static float buffer_angle[3]={0};
  volatile static int16_t buffer_gyro[3]={0};
  volatile static int16_t buffer_accel[3]={0};
  if (Calibration_times > 100 && Calibration_times <= 300)
  {
    ...
    buffer_angle[0] += Pitch;
    buffer_angle[1] += Roll;
    buffer_angle[2] += Yaw;
  }
  /*校准值计算*/
  if(Calibration_times==300)
  {
    calibration_value_gyro[0] = buffer_gyro[0] / 200;
    calibration_value_gyro[1] = buffer_gyro[1] / 200;
    calibration_value_gyro[2] = buffer_gyro[2] / 200;

    calibration_value_angle[0] = buffer_angle[0] / 200.0;
    calibration_value_angle[1] = buffer_angle[1] / 200.0;
    calibration_value_angle[2] = buffer_angle[2] / 200.0;

    printf("MPU6050 data calibration finished\r\n");
  }
  Calibration_times++;
}
